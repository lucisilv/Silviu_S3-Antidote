...
OLD */
    ...
    constructor() {
        ...
        var tailleTab=config.antidote.length;
       //process.stdout.write("\n tailletab="+tailleTab+" \n");
        //process.stdout.write("\n port0="+config.antidote[0].port+" \n");

        for(var i=0; i<tailleTab;i++){
          this.antidote[i] = antidoteClient.connect(config.antidote[i].port, config.antidote[i].host);
        //  process.stdout.write("\n\n TOUR BOUCLE NUMERO"+i+"\n\n");
          //process.stdout.write("  "+this.antidote[i].port);
          //process.stdout.write("  "+config.antidote[i].port);
...
chooseAntitode(bucketName,tailleTab)
{
        crc32 = Math.abs(CRC32.str(bucketName));
...
*/
    ...
    createBucket(bucketName, bucketMD, log, cb) {
        this.getBucketAttributes(bucketName, log, (err, bucket) => {
            ...
            const antidote_index = this.chooseAntitode(bucketName, tailleTab)
            this.antidoteCourant=this.antidote[antidote_index];
