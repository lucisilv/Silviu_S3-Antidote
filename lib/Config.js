...
original */
            ...
            if (config.antidoteTab) {

            var tailleTab=config.antidoteTab.length;

              for(var i=0; i<tailleTab;i++){

                if (config.antidoteTab[i].port !== undefined) {
                  assert(Number.isInteger(config.antidoteTab[i].port)
                  && config.antidoteTab[i].port > 0,
                  'bad config: vaultd port must be a positive integer');
                  this.antidote[i]={};
                  //process.stdout.write("\n Config.js :="+config.antidoteTab[i].port+" \n");

                  this.antidote[i].port =config.antidoteTab[i].port;
                //  process.stdout.write("\n Config.js this.antidote[i].port :="+this.antidote[i].port+" \n");

                  }
                  if (config.antidoteTab[i].host !== undefined) {
                    assert.strictEqual(typeof config.antidoteTab[i].host, 'string',
                      'bad config: vaultd host must be a string');
                    this.antidote[i].host = config.antidoteTab[i].host;
            			}
         			}

            //  process.stdout.write("\n Config.js this.antidote[0].host :="+this.antidote[0].host+" \n");
              //process.stdout.write("\n Config.js this.antidote[1].port :="+this.antidote[1].port+" \n");
              //process.stdout.write("\n Config.js this.antidote[2].port :="+this.antidote[2].port+" \n");
